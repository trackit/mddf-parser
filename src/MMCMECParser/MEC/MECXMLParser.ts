...
export function loadMECDataFromXml(xmlString: string): Promise<MECInterface> {
  ...
  if (basicNode) {
    ...
    mecData.Basic.LocalizedInfo = Array.from(localizedInfoNodes).map((localizedInfoNode) => {
      ...
      const genreArray = Array.from(genreNodes).map((genreNode) => {
        ...
        return {
          id, level, source, _tagText: genreText,
        };
      });
    ...
    mecData.Basic.ReleaseHistory = Array.from(releaseHistoryNodes).map((releaseHistoryNode) => {
      ...
      const DistrTerritory = {
        country: {
          _tagText: releaseHistoryNode.getElementsByTagName('md:DistrTerritory')[0]?.getElementsByTagName('md:country')[0]?.textContent ?? '',
        },
      };
      const Date = {
      ...
      const ReleaseOrg = {
        ...
        DisplayName: {
          _tagText: releaseHistoryNode.getElementsByTagName('md:ReleaseOrg')[0]?.getElementsByTagName('md:DisplayName')[0]?.textContent ?? '',
        },
        SortName: {
          _tagText: releaseHistoryNode.getElementsByTagName('md:ReleaseOrg')[0]?.getElementsByTagName('md:SortName')[0]?.textContent ?? '',
        },
        AlternateName: {
          _tagText: releaseHistoryNode.getElementsByTagName('md:ReleaseOrg')[0]?.getElementsByTagName('md:AlternateName')[0]?.textContent ?? '',
        },
      };

      return {
        ReleaseType, DistrTerritory, Date, Description, ReleaseOrg,
      };
    });
    ...
    if (RatingSetNode) {
      ...
      mecData.Basic.RatingSet.Rating = Array.from(RatingNodes).map((RatingNode) => {
        const Region = {
          country: { _tagText: RatingNode.getElementsByTagName('md:Region')[0]?.getElementsByTagName('md:country')[0]?.textContent ?? '' },
        };
    ...
    mecData.Basic.People = Array.from(PeopleNodes).map((PeopleNode) => {
      const JobNode = {
        ...
        BillingBlockOrder: {
          _tagText: PeopleNode.getElementsByTagName('md:Job')[0]?.getElementsByTagName('md:BillingBlockOrder')[0]?.textContent ?? '',
        },
      };
      const NameNode = {
        DisplayName: { _tagText: PeopleNode.getElementsByTagName('md:Name')[0]?.getElementsByTagName('md:DisplayName')[0]?.textContent ?? '' },
      };
  ...
  if (CompanyDisplayCreditNode) {
    mecData.CompanyDisplayCredit = {
      DisplayString: {
        language: CompanyDisplayCreditNode.getElementsByTagName('md:DisplayString')[0]?.getAttribute('language') ?? '',
        _tagText: CompanyDisplayCreditNode.getElementsByTagName('md:DisplayString')[0]?.textContent ?? '',
      },
    };
  }
